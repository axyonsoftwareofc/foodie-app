        ...
        <div className="space-y-4">
            ...
            <div className="p-4 rounded-2xl" style={{ backgroundColor: colors.bg }}>
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: colors.text }}>
                    ...
                    <div className="flex-1">
                        ...
                        {delivery.estimatedDeliveryTime && delivery.status !== 'DELIVERED' && (
                            <p className="text-xs mt-0.5 flex items-center gap-1" style={{ color: colors.text, opacity: 0.8 }}>
                                <Clock className="w-3 h-3" />
                                Previsao: {new Date(delivery.estimatedDeliveryTime).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                            </p>
                        )}
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-500">
                                {delivery.driver.vehicleType === 'MOTO' ? '🛵 Moto' :
                                 delivery.driver.vehicleType === 'BIKE' ? '🚲 Bicicleta' : '🚗 Carro'}
                ...
                <div className="space-y-3">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-700">{delivery.pickupAddress?.street}, {delivery.pickupAddress?.number}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-700">{delivery.deliveryAddress?.street}, {delivery.deliveryAddress?.number}</p>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="space-y-3">
                        {delivery.timeline.map((entry, i) => (
                            <div key={i} className="flex items-center gap-3">
                                ...
                                <span className="text-sm text-gray-700 flex-1">{STATUS_LABELS[entry.status] || entry.status}</span>
