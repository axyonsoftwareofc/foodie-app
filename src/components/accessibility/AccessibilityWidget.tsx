...
function isPresetActive(preset: AccessibilityPreset, settings: Record<string, unknown>): boolean {
  ...
  return Object.keys(presetSettings).every((key) => currentSettings[key] === presetSettings[key]);
}
            ...
            <div className="overflow-y-auto max-h-[calc(70vh-120px)] p-4">
              {activeTab === 'presets' ? (
                <div
                  className="space-y-2"
                  role="radiogroup"
                  aria-label="Selecionar perfil de acessibilidade"
                >
                  {presetOptions.map((preset) => (
                    ...
                    >
                      <span style={{ color: 'var(--color-primary)' }}>{preset.icon}</span>
