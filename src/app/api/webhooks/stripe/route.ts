    ...
    switch (event.type) {
      case 'payment_intent.succeeded': {
        ...
        if (orderId) {
          ...
          if (!result.success) {
            logger.warn('[Stripe Webhook] Failed to update order', {
              orderId,
              error: result.error,
            });
          }
        } else {
          logger.warn('[Stripe Webhook] PaymentIntent succeeded without orderId metadata', {
            paymentIntentId: paymentIntent.id,
          });
        }
        break;
      ...
      case 'payment_intent.payment_failed': {
        ...
        if (orderId) {
          ...
          if (!result.success) {
            logger.warn('[Stripe Webhook] Failed to update order on payment failure', {
              orderId,
              error: result.error,
            });
          }
