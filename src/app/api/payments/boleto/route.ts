...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to generate boleto' }, { status: 500 });
  }
}
...
export async function GET(request: NextRequest) {
  ...
  if (!boletoId) {
    return NextResponse.json({ error: 'Boleto ID is required' }, { status: 400 });
  }
