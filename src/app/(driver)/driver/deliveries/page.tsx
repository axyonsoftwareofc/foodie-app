...
export default function DriverDeliveriesPage() {
  ...
  const [proofForm, setProofForm] = useState<DeliveryProofForm>({
    photoUrl: '',
    notes: '',
    signedBy: '',
  });
      ...
      setSelectedDelivery((prev) =>
        prev
          ? {
              ...prev,
              status,
              timeline: [...prev.timeline, { status, timestamp: new Date().toISOString() }],
            }
          : null
      );
    } else {
  ...
  const getNextAction = (status: DeliveryStatus) => {
    switch (status) {
      case 'ASSIGNED':
        return { next: 'PICKED_UP', label: 'Confirmar Retirada', icon: <Package size={20} /> };
      case 'PICKED_UP':
        return { next: 'DELIVERING', label: 'Iniciar Entrega', icon: <Truck size={20} /> };
      case 'DELIVERING':
        return { next: 'DELIVERED', label: 'Confirmar Entrega', icon: <CheckCircle size={20} /> };
      default:
        return null;
    }
  };
          ...
          <div className="p-4 rounded-2xl" style={{ backgroundColor: 'var(--color-bg-card)' }}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                  {selectedDelivery.pickupAddress.number} -{' '}
                  {selectedDelivery.pickupAddress.neighborhood}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                  {selectedDelivery.deliveryAddress.number} -{' '}
                  {selectedDelivery.deliveryAddress.neighborhood}
                </p>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen pb-20" style={{ backgroundColor: 'var(--color-bg)' }}>
      <div className="p-4 text-center">
        ...
        <h2 className="font-bold mb-2" style={{ color: 'var(--color-text)' }}>
          Suas Entregas
        </h2>
