...
function KitchenOrderCard({
  ...
  const items = useMemo(() => {
    ...
    if (Array.isArray(raw))
      return raw as Array<{ menuItemName?: string; name?: string; quantity?: number }>;
    return [];
  }, [order]);
  ...
  const prepTime =
    ((order as unknown as Record<string, unknown>).estimatedPreparationTime as number) || 20;
  ...
  const typeLabel =
    order.orderType === 'DINE_IN'
      ? `Mesa ${(order as unknown as Record<string, unknown>).tableNumber || ''}`
  ...
  return (
    <div
      className={`rounded-xl border-2 p-4 flex flex-col gap-3 ${isNew ? 'animate-pulse border-amber-400 shadow-lg shadow-amber-200' : 'border-gray-200'} bg-white`}
    >
      {/* Header */}
      ...
      <div className="flex gap-2">
        {isNew && (
          <>
            <button
              ...
              className="flex-1 py-4 bg-emerald-600 text-white rounded-xl font-bold text-lg active:scale-95 transition-transform"
            >
              ✅ Aceitar
            </button>
            <button
              onClick={() => setShowCancel(true)}
              className="px-4 py-4 bg-red-50 text-red-600 rounded-xl font-bold active:scale-95 transition-transform"
            >
              ✕
            </button>
          </>
        ...
        {isReady && (
          <>
            {order.orderType === 'DELIVERY' && (
              <>
                <button
                  onClick={() => onAction('ASSIGN_DRIVER')}
                  className="flex-1 py-4 bg-purple-600 text-white rounded-xl font-bold text-lg active:scale-95 transition-transform"
                >
                  🛵 Despachar
                </button>
              </>
            ...
            {order.orderType === 'DINE_IN' && (
              <button
                onClick={() => onAction('COMPLETE_DINE_IN')}
                className="flex-1 py-4 bg-emerald-600 text-white rounded-xl font-bold text-lg active:scale-95 transition-transform"
              >
                🍽️ Servido
              </button>
            ...
            {order.orderType === 'PICKUP' && (
              <button
                onClick={() => onAction('COMPLETE_PICKUP')}
                className="flex-1 py-4 bg-orange-600 text-white rounded-xl font-bold text-lg active:scale-95 transition-transform"
              >
                🤝 Retirado
              </button>
            )}
        ...
        {isDelivering && (
          <button
            ...
            className="flex-1 py-4 bg-gray-600 text-white rounded-xl font-bold text-lg active:scale-95 transition-transform"
          >
            ✅ Concluir
          </button>
        )}
      ...
      {showCancel && (
        <div className="border-t border-red-200 pt-3 mt-1">
          ...
          <div className="flex flex-wrap gap-1 mb-2">
            {['Cliente desistiu', 'Sem ingredientes', 'Fora da area', 'Erro no pedido'].map((r) => (
              <button
                ...
                className={`text-xs px-2 py-1 rounded-full ${cancelReason === r ? 'bg-red-600 text-white' : 'bg-red-50 text-red-600'}`}
              >
                {r}
              </button>
            ))}
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {
                onCancel(cancelReason || 'Cancelado');
                setShowCancel(false);
              }}
              className="flex-1 py-2 bg-red-600 text-white rounded-lg font-medium text-sm"
            >
...
export default function CozinhaPage() {
  ...
  const handleCancel = useCallback(
    async (order: KitchenOrder, reason: string) => {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <header className="sticky top-0 z-40 bg-gray-900 text-white px-4 py-3 flex items-center justify-between shadow-lg">
        ...
        <button
          onClick={refresh}
          className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-sm font-medium active:scale-95 transition-transform"
        >
          Atualizar
        </button>
      </header>
        ...
        <div className="flex gap-3 p-3 min-h-full" style={{ minWidth: columns.length * 320 }}>
          {columns.map((col) => (
            <div key={col.id} className="flex-shrink-0 w-80 flex flex-col">
              {/* Column Header */}
              <div
                className={`sticky top-[57px] z-30 ${COLORS[col.id].bg} ${COLORS[col.id].border} border rounded-xl p-3 mb-3`}
              >
                <div className="flex items-center justify-between">
                  <h2 className={`font-bold ${COLORS[col.id].text}`}>{col.title}</h2>
                  <span
                    className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${COLORS[col.id].dot} text-white`}
                  >
                    {col.count}
                  </span>
                </div>
