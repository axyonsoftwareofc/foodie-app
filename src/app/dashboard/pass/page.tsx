...
export default function PassPage() {
    ...
    const handleComplete = useCallback(async (orderId: string, restaurantId: string, type: OrderType) => {
    ...
    return (
        <div className="min-h-screen bg-gray-100">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-400">
                            {readyCount} pedido{readyCount !== 1 ? 's' : ''} pronto{readyCount !== 1 ? 's' : ''} aguardando
                        </p>
                    </div>
                </div>
                <button onClick={refresh} className="px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium active:scale-95 transition-transform">
                    Atualizar
                </button>
            </header>
            ...
            {readyCount === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 h-[calc(100vh-73px)]">
                    {(Object.entries(ORDER_TYPE_CONFIG) as [OrderType, typeof ORDER_TYPE_CONFIG[OrderType]][]).map(([type, config]) => {
                        ...
                        return (
                            <div key={type} className={`rounded-2xl border-2 ${config.color} flex flex-col overflow-hidden`}>
                                {/* Column Header */}
                                ...
                                <div className="flex-1 overflow-y-auto p-3 space-y-3">
                                    {ordersOfType.map((order) => {
                                        ...
                                        const itemList = Array.isArray(items) ? items as Array<{ menuItemName?: string; name?: string; quantity?: number }> : []
                                        const tableNumber = (order as unknown as Record<string, unknown>).tableNumber as string
                                        const readyAt = (order as unknown as Record<string, unknown>).readyAt as string | null
                                        const wait = waitingMinutes(readyAt)

                                        return (
                                            <div key={order.id} className="bg-white rounded-xl border border-gray-200 p-4 flex flex-col gap-3 shadow-sm">
                                                {/* Header */}
                                                <div className="flex items-center justify-between">
                                                    <span className="text-xs font-mono text-gray-400">#{order.id.slice(-4)}</span>
                                                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full flex items-center gap-1 ${getWaitColor(wait)}`}>
                                                ...
                                                <div>
                                                    ...
                                                    {type === 'DINE_IN' && tableNumber && (
                                                        <p className="text-sm text-purple-600 font-medium">Mesa {tableNumber}</p>
                                                    )}
                                                </div>
                                                ...
                                                <div className="flex flex-wrap gap-1">
                                                    ...
                                                    {itemList.length > 4 && (
                                                        <span className="text-xs text-gray-400 self-end">+{itemList.length - 4}</span>
                                                    )}
                                                </div>
            ...
            <AssignDriverModal
                ...
                onClose={() => { setIsAssignModalOpen(false); setAssignOrderId(null) }}
                onAssigned={refresh}
            />
        </div>
    )
